...
=========================================================
*/
Particle::Particle( pugi::xml_node const &a_node, enum PoPs_class a_class, std::string const &a_family, int a_hasNucleus ) :
...
double Particle::massValue( char const *a_unit ) const {
    ...
    if( m_mass.size( ) == 0 ) throw std::runtime_error( "Particle does not have any mass data." );

    PQ_double const *pq_mass = dynamic_cast<PQ_double const *>( mass( )[0] );

    if( pq_mass == NULL ) throw std::runtime_error( "Particle does not have a PoPs::PQ_double mass." );

...
Nuclide::Nuclide( pugi::xml_node const &a_node, Database *a_DB, Isotope *a_isotope ) :
        Particle( a_node, class_nuclide, family_nuclide, -1 ),
        m_isotope( a_isotope ),
...
double Nuclide::massValue( char const *a_unit ) const {
    ...
    if( mass( ).size( ) > 0 ) {
        ...
        if( baseMass( ).size( ) == 0 ) throw std::out_of_range( "nuclide::massValue: no mass in level 0." );
...
=========================================================
*/
void Nuclide::calculateNuclideGammaBranchStateInfos( PoPs::Database const &a_pops, NuclideGammaBranchStateInfos &a_nuclideGammaBranchStateInfos ) const {

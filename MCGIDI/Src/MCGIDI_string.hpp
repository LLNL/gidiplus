...
class String
{
  ...
  public:
    ...
    MCGIDI_HOST_DEVICE String();
    MCGIDI_HOST_DEVICE ~String();
    MCGIDI_HOST_DEVICE String(const String&);
    MCGIDI_HOST_DEVICE String(const char*);

    MCGIDI_HOST_DEVICE String&  operator=(const char*);
    MCGIDI_HOST_DEVICE String&  operator=(const String&);

    MCGIDI_HOST_DEVICE String&  operator+=(const String&);
    MCGIDI_HOST_DEVICE String&  operator+=(const char*);
    MCGIDI_HOST_DEVICE String&  operator+=(char);
    MCGIDI_HOST_DEVICE void     push_back(char);

    friend String
    MCGIDI_HOST_DEVICE operator+(const String& lhs, const String& rhs);

    MCGIDI_HOST_DEVICE bool     operator==(const char*) const;
    MCGIDI_HOST_DEVICE bool     operator==(const String&) const;

    MCGIDI_HOST_DEVICE void     clear();       // set string to empty string (memory remains reserved)
    MCGIDI_HOST_DEVICE void     clearMemory(); // set string to empty string (memory is free'd)

    MCGIDI_HOST_DEVICE size_type size()   const   { return size_; }   ///< size without terminating NUL
    MCGIDI_HOST_DEVICE size_type length() const   { return size_; }   ///< as size()

    // size if fully used
    MCGIDI_HOST_DEVICE size_type capacity() const { return allocated_-1; }

    // 8 byte alligned size
    MCGIDI_HOST_DEVICE long internalSize() const {
    ...
    MCGIDI_HOST_DEVICE bool      empty() const    { return size_ == 0; }

    MCGIDI_HOST_DEVICE const char*  c_str() const { return p; } ///< raw data
    ...
    /** Reserve internal string memory so that n characters can be put into the
        ...
        amount.
        */
    MCGIDI_HOST_DEVICE void reserve( size_type n, char ** address = nullptr);
    ...
    /** Resize string. If n is less than the current size, the string will be truncated.
        ...
        */
    MCGIDI_HOST_DEVICE void resize( size_type n, char ** address = nullptr);

    /** Resize string. If n is less than the current size, the string will be truncated.
        If n is larger, then the memory will be reallocated to exactly this amount, and
        the additional characters will be c characters.
        */
    MCGIDI_HOST_DEVICE void resize( size_type n, char c, char ** address = nullptr);

    /// swap contents
    MCGIDI_HOST_DEVICE void swap( String& );

    MCGIDI_HOST_DEVICE String   substr(const size_type pos, size_type length) const;

    // unchecked access:
    MCGIDI_HOST_DEVICE char&    operator[](const size_type i)       { return p[i]; }
    MCGIDI_HOST_DEVICE char     operator[](const size_type i) const { return p[i]; }
    // checked access:
    MCGIDI_HOST_DEVICE char&    at(const size_type i);
    MCGIDI_HOST_DEVICE char     at(const size_type i) const;

    /// erase len characters at position pos
    MCGIDI_HOST_DEVICE String& erase(size_type pos, size_type len);
    /// Append n characters of a string
    MCGIDI_HOST_DEVICE String& append(const char* str, size_type n);

    MCGIDI_HOST_DEVICE int     compare( size_type pos, size_type len, const String& str ) const;
    MCGIDI_HOST_DEVICE int     compare( size_type pos, size_type len, const char*   str ) const;

  private:
    // reallocate the internal memory
    MCGIDI_HOST_DEVICE void  my_realloc( size_type n, char ** address = nullptr);
    MCGIDI_HOST_DEVICE char* strdup_never_null(const char* other);

};
// class

MCGIDI_HOST_DEVICE bool operator<(const String&, const String&);

...
Regions1d::Regions1d( Construction::Settings const &a_construction, pugi::xml_node const &a_node, Suite *a_parent ) :
        Function1dForm( a_construction, a_node, f_regions1d, a_parent ) {
    ...
    for( pugi::xml_node child = a_node.first_child( ); child; child = child.next_sibling( ) ) {
        ...
        if( _form == NULL ) throw std::runtime_error( "Regions1d::Regions1d: data1dParse returned NULL." );
        append( _form );
    }
}
...
double Regions1d::domainMin( ) const {
    ...
    if( m_Xs.size( ) == 0 ) throw std::runtime_error( "Regions1d::domainMin: Regions1d has no regions" );
    return( m_Xs[0] );
}
...
double Regions1d::domainMax( ) const {
    ...
    if( m_Xs.size( ) == 0 ) throw std::runtime_error( "Regions1d::domainMax: Regions1d has not regions" );
...
void Regions1d::append( Function1dForm *a_function ) {
    ...
    if( dimension( ) != a_function->dimension( ) ) throw std::runtime_error( "Regions1d::append: dimensions differ." );
    ...
    if( m_Xs.size( ) == 0 ) {
        ...
        if( m_Xs.back( ) != _domainMin ) throw std::runtime_error( "Regions1d::append: regions do not abut." );
    }
...
double Regions1d::evaluate( double a_x1 ) const {
    ...
    if( m_Xs.size( ) == 0 ) throw std::runtime_error( "Regions1d::evaluate: Regions1d has not regions" );

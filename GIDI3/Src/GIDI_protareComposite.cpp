...
/* *********************************************************************************************************//**
 * Returns the number of **ProtareSingleton**s contained in *this*.
 *
 * @return                              Returns the number of contained **ProtareSingleton**s..
 ***********************************************************************************************************/
 ...
 *
 * @param a_index               [in]    Index of the **ProtareSingleton** to return.
 *
 * @return                              Pointer to the requested protare or NULL if invalid *a_index*..
 ***********************************************************************************************************/

ProtareSingleton const *ProtareComposite::protare( std::size_t a_index ) const {
...
Documentation::Suite const &ProtareComposite::documentations( ) const {
...
Styles::Base const &ProtareComposite::style( std::string const a_label ) const {
...
void ProtareComposite::productIDs( std::set<std::string> &a_ids, Settings::Particles const &a_particles, bool a_transportablesOnly ) const {
...
int ProtareComposite::maximumLegendreOrder( Settings::MG const &a_settings, std::string const &a_productID ) const {

    int maximumLegendreOrder1 = -1;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) {
        int maximumLegendreOrder2 = m_protares[i1]->maximumLegendreOrder( a_settings, a_productID );
...
Reaction const *ProtareComposite::reaction( std::size_t a_index ) const {
    ...
    throw std::runtime_error( "ProtareComposite::reaction: index out of range" );
}
...
Reaction const *ProtareComposite::orphanProduct( std::size_t a_index ) const {
    ...
    throw std::runtime_error( "ProtareComposite::orphanProduct: index out of range" );
}
...
std::vector<double> const &ProtareComposite::groupBoundaries( Settings::MG const &a_settings, std::string const &a_productID ) const {

    return( m_protares[0]->groupBoundaries( a_settings, a_productID ) );
}
...
Vector ProtareComposite::multiGroupInverseSpeed( Settings::MG const &a_settings, Settings::Particles const &a_particles ) const {

    return( m_protares[0]->multiGroupInverseSpeed( a_settings, a_particles ) );
}
...
Vector ProtareComposite::multiGroupCrossSection( Settings::MG const &a_settings, Settings::Particles const &a_particles ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupCrossSection( a_settings, a_particles );
...
Vector ProtareComposite::multiGroupQ( Settings::MG const &a_settings, Settings::Particles const &a_particles, bool a_final ) const {

    Vector vector;
    
    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupQ( a_settings, a_particles, a_final );
...
Vector ProtareComposite::multiGroupMultiplicity( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupMultiplicity( a_settings, a_particles, a_productID );
...
Vector ProtareComposite::multiGroupFissionNeutronMultiplicity( Settings::MG const &a_settings, Settings::Particles const &a_particles ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupFissionNeutronMultiplicity( a_settings, a_particles );
...
Matrix ProtareComposite::multiGroupProductMatrix( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID, int a_order ) const {

    Matrix matrix( 0, 0 );

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) matrix += m_protares[i1]->multiGroupProductMatrix( a_settings, a_particles, a_productID, a_order );
...
Matrix ProtareComposite::multiGroupFissionMatrix( Settings::MG const &a_settings, Settings::Particles const &a_particles, int a_order ) const {

    Matrix matrix( 0, 0 );

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) matrix += m_protares[i1]->multiGroupFissionMatrix( a_settings, a_particles, a_order );
...
Vector ProtareComposite::multiGroupTransportCorrection( Settings::MG const &a_settings, Settings::Particles const &a_particles, int a_order, transportCorrectionType a_transportCorrectionType, double a_temperature ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupTransportCorrection( a_settings, a_particles, a_order, a_transportCorrectionType, a_temperature );
...
Vector ProtareComposite::multiGroupAvailableEnergy( Settings::MG const &a_settings, Settings::Particles const &a_particles ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupAvailableEnergy( a_settings, a_particles );
...
Vector ProtareComposite::multiGroupAverageEnergy( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupAverageEnergy( a_settings, a_particles, a_productID );
...
Vector ProtareComposite::multiGroupDepositionEnergy( Settings::MG const &a_settings, Settings::Particles const &a_particles ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupDepositionEnergy( a_settings, a_particles );
...
Vector ProtareComposite::multiGroupAvailableMomentum( Settings::MG const &a_settings, Settings::Particles const &a_particles ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupAvailableMomentum( a_settings, a_particles );
...
Vector ProtareComposite::multiGroupAverageMomentum( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupAverageMomentum( a_settings, a_particles, a_productID );
...
Vector ProtareComposite::multiGroupDepositionMomentum( Settings::MG const &a_settings, Settings::Particles const &a_particles ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupDepositionMomentum( a_settings, a_particles );
...
Vector ProtareComposite::multiGroupGain( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {

    Vector vector;

    for( std::size_t i1 = 0; i1 < m_protares.size( ); ++i1 ) vector += m_protares[i1]->multiGroupGain( a_settings, a_particles, a_productID );

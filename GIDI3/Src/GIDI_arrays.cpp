...
int parseFlattened1d( Construction::Settings const &a_construction, pugi::xml_node const &a_node, Vector &a_data ) {
    ...
    for( std::size_t i1 = 0; i1 < arrayData.m_numberOfStarts; ++i1 ) {
        ...
        for( int32_t i2 = 0; i2 < arrayData.m_lengths[i1]; ++i2, ++n1, ++offset ) {
            if( n1 >= arrayData.m_dValues.size( ) ) throw std::runtime_error( "Too many values in flattened array." );
            if( offset >= n2 ) throw std::runtime_error( "requested size is too small." );
...
FlattenedArrayData::FlattenedArrayData( pugi::xml_node const &a_node, int a_dimensions, int a_useSystem_strtod ) :
        Form( a_node, f_flattenedArrayData ),
        m_numberOfStarts( 0 ), 
        m_numberOfLengths( 0 ),
        m_starts( NULL ),
        m_lengths( NULL ) {
    ...
    if( a_dimensions != (int) m_shape.size( ) ) throw std::runtime_error( "a_dimensions != m_shape.size( )" );
    ...
    for( pugi::xml_node child = a_node.first_child( ); child; child = child.next_sibling( ) ) {
        ...
        if( name == "values" ) {
            ...
            if( label == "starts" ) {
                ...
                if( m_starts == NULL ) throw std::runtime_error( "m_starts == NULL" );
                if( *endCharacter != 0 ) throw std::runtime_error( "bad 'starts' string" ); } 
                ...
                if( m_lengths == NULL ) throw std::runtime_error( "m_lengths == NULL" );
                if( *endCharacter != 0 ) throw std::runtime_error( "bad 'lengths' string" ); }
            else if( label == "" ) {
                m_dValuesPresent = true;
                parseValuesOfDoubles( child, m_dValues, a_useSystem_strtod ); 
}
            else {
                throw std::runtime_error( "unknown label for flatteded array sub-element" );
            } }
        else {
            throw std::runtime_error( "unknown flattened array sub-element" );
        }
    }
    if( m_starts == NULL ) throw std::runtime_error( "array missing starts element" );
    if( m_lengths == NULL ) throw std::runtime_error( "array missing lengths element" );
    if( !m_dValuesPresent ) throw std::runtime_error( "array missing dValues element" );
    if( m_numberOfStarts != m_numberOfLengths ) throw std::runtime_error( "m_numberOfStarts != m_numberOfLengths for array" );
}

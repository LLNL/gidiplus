...
Vector collapse( Vector const &a_vector, Settings::Settings const &a_settings, Settings::Particles const &a_particles, double a_temperature ) {
    ...
    Settings::Particle const *projectile( a_particles.particle( a_settings.projectileID( ) ) );
    Settings::ProcessedFlux const *flux( projectile->nearestProcessedFluxToTemperature( a_temperature ) );
...
Matrix collapse( Matrix const &a_matrix, Settings::Settings const &a_settings, Settings::Particles const &a_particles, double a_temperature, std::string const &a_productID ) {
    ...
    Settings::Particle const *product( a_particles.particle( a_productID ) );
...
Matrix transportCorrect( Matrix const &a_matrix, Vector const &a_transportCorrection ) {
    ...
    if( a_matrix.size( ) == 0 ) {
        ...
        if( size != a_matrix.size( ) ) throw std::runtime_error( "transportCorrect: matrix rows different than vector size." );
    }
...
Vector multiGroupXYs1d( Settings::MultiGroup const &a_boundaries, XYs1d const &a_function, Settings::Flux const &a_flux ) {
    ...
    if( boundaries_xs == NULL ) throw std::runtime_error( "GIDI::multiGroup: ptwX_create failed." );

    Settings::Flux_order const &flux_order_0 = a_flux[0];
    ...
    if( fluxes_xys == NULL ) {
        ...
        throw std::runtime_error( "GIDI::multiGroup: ptwXY_createFrom_Xs_Ys failed." );
    }
    ...
    if( multiGroupFlux == NULL ) {
        ...
        throw std::runtime_error( "GIDI::multiGroup: ptwXY_groupOneFunction failed." );
    }
    ...
    if( groups == NULL ) throw std::runtime_error( "GIDI::multiGroup: ptwXY_groupTwoFunctions failed." );

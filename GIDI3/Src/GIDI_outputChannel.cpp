...
void OutputChannel::productIDs( std::set<std::string> &a_indices, Settings::Particles const &a_particles, bool a_transportablesOnly ) const {
...
int OutputChannel::maximumLegendreOrder( Settings::MG const &a_settings, std::string const &a_productID ) const {
    ...
    for( std::size_t index = 0; index < size; ++index ) {
        Product const &_product = *m_products.get<Product>( index );
        int r_maximumLegendreOrder = _product.maximumLegendreOrder( a_settings, a_productID );

        if( r_maximumLegendreOrder > _maximumLegendreOrder ) _maximumLegendreOrder = r_maximumLegendreOrder;
    }

    int r_maximumLegendreOrder = m_fissionFragmentData.maximumLegendreOrder( a_settings, a_productID );
    if( r_maximumLegendreOrder > _maximumLegendreOrder ) _maximumLegendreOrder = r_maximumLegendreOrder;
...
Vector OutputChannel::multiGroupMultiplicity( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {
    ...
    for( std::size_t index = 0; index < m_products.size( ); ++index ) {
        ...
        vector += _product.multiGroupMultiplicity( a_settings, a_particles, a_productID );
    }

    vector += m_fissionFragmentData.multiGroupMultiplicity( a_settings, a_particles, a_productID );
...
Vector OutputChannel::multiGroupQ( Settings::MG const &a_settings, Settings::Particles const &a_particles, bool a_final ) const {

    Vector vector( 0 );

    Form const &_form = *m_Q.getViaLineage<Form>( a_settings.label( ) );
    Gridded1d const &_gridded1d = dynamic_cast<Gridded1d const &>( _form );

    vector += _gridded1d.data( );

    if( a_final ) {
        for( std::size_t index = 0; index < m_products.size( ); ++index ) {
            Product const &_product = *m_products.get<Product>( index );

            vector += _product.multiGroupQ( a_settings, a_particles, a_final );
        }
    }

    vector += m_fissionFragmentData.multiGroupQ( a_settings, a_particles, a_final );
...
Matrix OutputChannel::multiGroupProductMatrix( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID, int a_order ) const {
    ...
    for( std::size_t index = 0; index < m_products.size( ); ++index ) {
        ...
        matrix += _product.multiGroupProductMatrix( a_settings, a_particles, a_productID, a_order );
    }

    matrix += m_fissionFragmentData.multiGroupProductMatrix( a_settings, a_particles, a_productID, a_order ); 
...
Vector OutputChannel::multiGroupAverageEnergy( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {
    ...
    for( std::size_t index = 0; index < m_products.size( ); ++index ) {
        ...
        vector += _product.multiGroupAverageEnergy( a_settings, a_particles, a_productID );
    }

    vector += m_fissionFragmentData.multiGroupAverageEnergy( a_settings, a_particles, a_productID ); 
...
Vector OutputChannel::multiGroupAverageMomentum( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {
    ...
    for( std::size_t index = 0; index < m_products.size( ); ++index ) {
        ...
        vector += _product.multiGroupAverageMomentum( a_settings, a_particles, a_productID );
    }

    vector += m_fissionFragmentData.multiGroupAverageMomentum( a_settings, a_particles, a_productID ); 

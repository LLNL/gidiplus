...
FissionFragmentData::FissionFragmentData( Construction::Settings const &a_construction, pugi::xml_node const &a_node, PoPs::Database const &a_pops, 
    ...
    for( std::size_t i1 = 0; i1 < m_delayedNeutrons.size( ); ++i1 ) {
        DelayedNeutron *delayedNeutron = m_delayedNeutrons.getNonConst<DelayedNeutron>( i1 );

        delayedNeutron->delayedNeutronIndex( i1 );
    }
}
...
void FissionFragmentData::productIDs( std::set<std::string> &a_indices, Settings::Particles const &a_particles, bool a_transportablesOnly ) const {
...
int FissionFragmentData::maximumLegendreOrder( Settings::MG const &a_settings, std::string const &a_productID ) const {

    int _maximumLegendreOrder = -1;

    for( std::size_t index = 0; index < m_delayedNeutrons.size( ); ++index ) {
        DelayedNeutron const &delayedNeutrons1 = *m_delayedNeutrons.get<DelayedNeutron>( index );
        int r_maximumLegendreOrder = delayedNeutrons1.maximumLegendreOrder( a_settings, a_productID );
...
Vector FissionFragmentData::multiGroupMultiplicity( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {

    Vector vector( 0 );

    if( !a_settings.delayedNeutrons( ) ) return( vector );

    for( std::size_t index = 0; index < m_delayedNeutrons.size( ); ++index ) {
        DelayedNeutron const &delayedNeutrons1 = *m_delayedNeutrons.get<DelayedNeutron>( index );

        vector += delayedNeutrons1.multiGroupMultiplicity( a_settings, a_particles, a_productID );
    }
...
Vector FissionFragmentData::multiGroupQ( Settings::MG const &a_settings, Settings::Particles const &a_particles, bool a_final ) const {
    ...
    FissionEnergyRelease const &fissionEnergyRelease1d = *m_fissionEnergyReleases.getViaLineage<FissionEnergyRelease>( a_settings.label( ) );

    vector += fissionEnergyRelease1d.multiGroupQ( a_settings, a_particles );
...
Matrix FissionFragmentData::multiGroupProductMatrix( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID, int a_order ) const {

    Matrix matrix( 0, 0 );

    if( !a_settings.delayedNeutrons( ) ) return( matrix );

    for( std::size_t index = 0; index < m_delayedNeutrons.size( ); ++index ) {
        DelayedNeutron const &delayedNeutrons1 = *m_delayedNeutrons.get<DelayedNeutron>( index );

        matrix += delayedNeutrons1.multiGroupProductMatrix( a_settings, a_particles, a_productID, a_order );
    }
...
Vector FissionFragmentData::multiGroupAverageEnergy( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {
    ...
    for( std::size_t index = 0; index < m_delayedNeutrons.size( ); ++index ) {
        ...
        vector += delayedNeutrons1.multiGroupAverageEnergy( a_settings, a_particles, a_productID );
    }
...
Vector FissionFragmentData::multiGroupAverageMomentum( Settings::MG const &a_settings, Settings::Particles const &a_particles, std::string const &a_productID ) const {
    ...
    for( std::size_t index = 0; index < m_delayedNeutrons.size( ); ++index ) {
        ...
        vector += delayedNeutrons1.multiGroupAverageMomentum( a_settings, a_particles, a_productID );
    }

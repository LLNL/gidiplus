...
Flux_order::Flux_order( int a_order, std::vector<double> const &a_energies, std::vector<double> const &a_fluxes ) :
    ...
    if( a_energies.size( ) != a_fluxes.size( ) ) throw std::runtime_error( "Flux_order::Flux_order: a_energies.size( ) != a_fluxes.size( )." );
}
...
*/
    ...
    if( order > (int) m_fluxOrders.size( ) ) throw std::runtime_error( "Flux::addFluxOrder: order > (int) m_fluxOrders.size( )." );
...
*/
    ...
    if( boundaries == NULL ) throw std::runtime_error( "ptwX_create failted for boundaries." );
    ...
    for( ; i1 < 1; ++i1 ) {     // only do l=0 currenlty hence ' i1 < 1' test.
        ...
        if( __flux == NULL ) throw std::runtime_error( "ptwXY_createFrom_Xs_Ys failed for __flux." );

        ptwXPoints *groupedFluxX = ptwXY_groupOneFunction( NULL, __flux, boundaries, ptwXY_group_normType_none, NULL );
        if( groupedFluxX == NULL ) throw std::runtime_error( "ptwXY_groupOneFunction failed for groupedFluxX." );
...
void Fluxes_from_bdfls::initialize( char const *a_fileName, double a_temperature_MeV ) {
    ...
    if( fIn == NULL ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: Could not open bdfls file." );

    while( true ) {                 // Skip over groups.
        if( fgets( buffer, 132, fIn ) == NULL ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: fgets failed for fid." );
        if( strlen( buffer ) > 73 ) {
    ...
    while( true ) {
        ...
        if( fid == -1 ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: converting fid to long failed." );
        std::string label( LLNL_fidToLabel( fid ) );
        Flux flux( label, a_temperature_MeV );

        long maximumFluxOrder( -1 );
        if( fgets( buffer, 132, fIn ) == NULL ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: fgets failed for maximumFluxOrder." );
        maximumFluxOrder = strtol( buffer, &pEnd, 10 );
        if( maximumFluxOrder == -1 ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: converting maximumFluxOrder to long failed." );
        if( maximumFluxOrder >= (long) ( sizeof( numberOfValuesInOrders ) / sizeof( numberOfValuesInOrders[0] ) ) )
            throw std::runtime_error( "Fluxes_from_bdfls::initialize: need to increase size of numberOfValuesInOrders" );

        for( long order = 0; order <= maximumFluxOrder; ++order ) {
            numberOfValuesInOrders[order] = -1;
            if( fgets( buffer, 132, fIn ) == NULL ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: fgets failed for maximumFluxOrders." );
            numberOfValuesInOrders[order] = strtol( buffer, &pEnd, 10 );
            if( numberOfValuesInOrders[order] == -1 ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: converting numberOfValuesInOrders[order] to long failed." );
        ...
        for( long order = 0; order <= maximumFluxOrder; ++order ) {
            ...
            while( numberOfValuesInOrder > 0 ) {
                ...
                if( fgets( buffer, 132, fIn ) == NULL ) throw std::runtime_error( "Fluxes_from_bdfls::initialize: fgets failed for energies/fluxes." );
...
Flux Fluxes_from_bdfls::getViaFID( int a_fid ) const {
    ...
    throw std::runtime_error( "Fluxes_from_bdfls::getViaFID: fid not found." );
}

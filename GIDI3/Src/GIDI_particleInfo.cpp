...
/*! \class ParticleInfo
 * This class stores an abridged set of particle information from PoPs::Database as needed by GIDI.
 *
...
/* *********************************************************************************************************//**
 * @param a_ID                      [in]    The particle's PoPs ID.
 * @param a_globalPoPs              [in]    A PoPs::Database instance used to get particle indices and possibly other particle information.
...
ParticleInfo::ParticleInfo( std::string const &a_ID, PoPs::Database const &a_globalPoPs, PoPs::Database const &a_internalPoPs, bool a_requiredInGlobalPoPs ) :
        m_id( ParticleInfo::IDPortion( a_ID ) ),
        m_qualifier( ParticleInfo::qualifierPortion( a_ID ) ),
        m_pid( "" ),
        m_mass( -1, "amu" ),
    ...
    PoPs::Base const *particleOrAlias = NULL;       // Need to get the mass and nuclear excitation energy. Favor from internal PoPs if present.
    std::string energyUnit( "MeV" );

    if( a_globalPoPs.exists( m_id ) ) {
        particleOrAlias = &a_globalPoPs.get<PoPs::Base>( m_id );
        if( particleOrAlias->isAlias( ) ) {
            PoPs::Alias const *alias = static_cast<PoPs::Alias const *>( particleOrAlias );
            particleOrAlias = &a_globalPoPs.get<PoPs::Base>( alias->pid( ) );
        }
        m_pid = particleOrAlias->ID( ); }
    else {
        if( a_requiredInGlobalPoPs ) throw std::runtime_error( "ParticleInfo::ParticleInfo: required particle ID not in global PoPs: " + m_id );
    }

    if( a_internalPoPs.exists( m_id ) ) {
        particleOrAlias = &a_internalPoPs.get<PoPs::Base>( m_id );
        if( particleOrAlias->isAlias( ) ) {
            PoPs::Alias const *alias = static_cast<PoPs::Alias const *>( particleOrAlias );
            particleOrAlias = &a_globalPoPs.get<PoPs::Base>( alias->pidIndex( ) );
        }
    }

    if( particleOrAlias == NULL ) throw std::runtime_error( "ParticleInfo::ParticleInfo: particle ID not in global PoPs: " + m_id );

    if( particleOrAlias->isParticle( ) ) {
        PoPs::Particle const &particle = static_cast<PoPs::Particle const &>( *particleOrAlias );
        ...
        if( particle.isNuclide( ) ) {
            PoPs::Nuclide const &nuclide = static_cast<PoPs::Nuclide const &>( particle );
...
double ParticleInfo::mass( std::string const &a_unit ) const {
    ...
    return( PoPs_AMU2MeV_c2 * m_mass.value( ) );
}

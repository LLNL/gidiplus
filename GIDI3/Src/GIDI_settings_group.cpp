...
void Groups_from_bdfls::initialize( char const *a_fileName ) {
    ...
    if( fIn == NULL ) throw std::runtime_error( "Groups_from_bdfls::initialize: Could not open bdfls file." );

    while( true ) {
        int gid( -1 );
        if( fgets( buffer, 132, fIn ) == NULL ) throw std::runtime_error( "Groups_from_bdfls::initialize: fgets failed for gid." );
        if( strlen( buffer ) > 73 ) {
            if( buffer[72] == '1' ) break;
        }
        gid = (int) strtol( buffer, &pEnd, 10 );
        if( gid == -1 ) throw std::runtime_error( "Groups_from_bdfls::initialize: converting gid to long failed." );
        std::string label( LLNL_gidToLabel( gid ) );

        long numberOfBoundaries( -1 );
        if( fgets( buffer, 132, fIn ) == NULL ) throw std::runtime_error( "Groups_from_bdfls::initialize: fgets failed for numberOfBoundaries." );
        numberOfBoundaries = strtol( buffer, &pEnd, 10 );
        if( numberOfBoundaries == -1 ) throw std::runtime_error( "Groups_from_bdfls::initialize: converting gid to long failed." );
        ...
        while( numberOfBoundaries > 0 ) {
            ...
            if( fgets( buffer, 132, fIn ) == NULL ) throw std::runtime_error( "Groups_from_bdfls::initialize: fgets failed for boundaries." );
...
MultiGroup Groups_from_bdfls::viaLabel( std::string const &a_label ) const {
    ...
    throw std::runtime_error( "Groups_from_bdfls::viaLabel: label not found." );
}

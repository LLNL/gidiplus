...
Matrix &Matrix::operator+=( Matrix const &a_rhs ) {
    ...
    if( size( ) != a_rhs.size( ) ) throw std::runtime_error( "matrix sizes differ." );
    if( m_matrix[0].size( ) != a_rhs[0].size( ) ) throw std::runtime_error( "matrix colums numbers differ." );
...
void Matrix::push_back( Vector const &a_vector ) {
    ...
    if( size( ) > 0 ) {
        if( (*this)[0].size( ) != a_vector.size( ) ) throw std::runtime_error( "matrix::push_back: size different" );
    }

...
int main2( int argc, char **argv ) {
    ...
    GIDI::Vector vector = protare->multiGroupInverseSpeed( settings, temperatures[0] );
    printVector( "Inverse speed :: ", vector );

    vector = protare->multiGroupCrossSection( settings, temperatures[0] );
    ...
    for( int order = 0; order < protare->maximumLegendreOrder( settings, temperatures[0], PoPI::IDs::photon ); ++order ) {
        ...
        GIDI::Matrix matrix = protare->multiGroupProductMatrix( settings, temperatures[0], particles, PoPI::IDs::neutron, order );

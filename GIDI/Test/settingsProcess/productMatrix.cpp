...
int main( int argc, char **argv ) {
    ...
    if( argc > 2 ) delayedNeutrons = GIDI::Transporting::DelayedNeutrons::on;
    ...
    for( std::size_t index = 0; index < protare->numberOfReactions( ); ++index ) {
        ...
        int maxOrder = reaction->maximumLegendreOrder( settings, temperatures[0], projectileID );
        GIDI::Matrix m1 = reaction->multiGroupProductMatrix( settings, temperatures[0], particles, projectileID, 0 );
